...
fire_data <- read.csv(file = here::here("data/02-analysis_data/tfs_analysis_data"))
...
#| warning: false
library(ggplot2)
fire_data <- read.csv(here::here("data/02-analysis_data/tfs_analysis_data"))
...
fire_data_model <- read_csv( file = here::here("data/02-analysis_data/severity_tfs_analysis_data"))
...
#### Read data ####
fire_data_model <- read_csv(file = here::here("data/02-analysis_data/updated_tfs_analysis_data"))
...
fire_data_current <- read_csv( file = here::here("data/02-analysis_data/severity_tfs_analysis_data"))
...
## Government and Agency Actions for Improving Fire Safety Infrastructure {#sec-second-point}
...
## TFS Decision Making {#sec-third-point}
...
fire_data_model <- read_csv(here::here("data/02-analysis_data/severity_tfs_analysis_data"))
...
This appendix contains all plot/summary distribution of all variables in intrested choose in analysis data set.

1.  Are of Origin (see @fig-area-origin-distribution)
2.  Civilian Casualties (see @tbl-civilian-casualties-summary)
3.  Estimated Dollar Loss (see @tbl-estimatedloss)
...
4.  Final Incidence Type (see @fig-final-incident-type-distribution)
5.  Fire Alram Conditions (see @fig-firealarmsystem)
...
6.  Ignition Source (see @fig-grouped-ignition-source-distribution)
7.  Number of Responding Apparatus (See @fig-responding-apparatus)
...
8.  Number of Responding Personnel (See @fig-responding-personnel-hist)
...
9.  Possible Cause (See @tbl-possible-cause-full)
...
11. Sprinkler System Presence (@tbl-sprinkler-system)
...
12. tfs_alarm_time (Amount of Fire Incidents by Year, see @fig-fire-incidents-by-year) (Fire Incidents by Month and Year, See @fig-year-month-incident-count)
...
#| fig-caption: This bar chart shows the number of fire incidents by year, from 2011 to 2023.
## 12. tfs_alarm_time ## 
...
#| tbl-cap: Monthly Incident Count by Year (Excluding 2022 and 2023 becasue from prvious plot, we can see they don't contribute much information)
...
14. Firefighter Casualties (See @fig-firefighter-casulties)
...
15. Response Time (See @fig-response-time-distribution)
...
The fire incidence dataset includes several categorical variables such as Ignition Source, Possible Cause, and Initial Types. During data collection, these variables, which have a wide range of categories, may introduce various types of bias.
...
### Sampling Method in Fire Incident Data
