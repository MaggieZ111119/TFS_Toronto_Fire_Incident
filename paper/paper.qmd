...
subtitle: "Key Role of Origin of Fire and Opportunities to Improve the Effectiveness of Toronto's Public Fire Infrastructure"
author: 
  - Maggie Zhang
thanks: "Code and data supporting this analysis are available at: [https://github.com/MaggieZ111119/TFS_Toronto_Fire_Incident](https://github.com/MaggieZ111119/TFS_Toronto_Fire_Incident)."
date: today
date-format: long
abstract: "This study investigates fire severity levels in Toronto to improve resource allocation for Toronto Fire Services (TFS). The analysis identifies that higher-severity incidents are strongly correlated with fires originating in mechanical, HVAC, and electrical areas, while cooking and lighting areas tend to lead to more frequent but lower-severity fires.It also finds that fire incidents involving certain ignition sources, such as electrical units or lightning equipment, are more likely to cause severe fires. Additionally, the study evaluates the effectiveness of public fire response systems, highlighting that fire alarms are effective in low and medium-severity fires, while sprinkler systems are often only partially operational. These findings can assist TFS in enhancing response strategies, prioritizing resources, and optimizing fire response operations, ultimately improving community safety in Toronto."
format: pdf
...
The study aims to estimate the severity of fires in the Toronto city A fire incident, as defined by the Ontario Fire Marshal (OFM), refers to any event involving fire, explosion, or other situations suspected to lead to significant outcomes, such as injury or death [@ontario_fire_marshal].

The severity of a fire can be assessed through various factors, including the intensity of the flames, the size of the affected area, its impact on human activities, and its potential threat to animal habitats. There are numerous standards and considerations when discussing fire severity, but when it comes to prediction, it is unrealistic to consider all factors. Therefore, defining severity becomes the first task before any further analysis. In this study, the severity of a fire is defined based on three aspects: casualty and financial loss, the type of incident, and the number of apparatuses dispatched by Toronto Fire Services (TFS) in response to the fire. Further explanation of how severity is defined will be provided later in the paper.

This study also identifies high-priority response criteria that can assist in enhancing TFS' future operations. By examining the data, it has been found that the area of origin of a fire is a significant predictor of fire severity. Fires originating from mechanical, HVAC, and electrical areas are highly correlated with severe incidents, whereas fires originating from cooking and heating areas are more commonly associated with low-severity incidents though they are more frequent.

Additionally, this study explores the effectiveness of current public fire response mechanisms in Toronto, such as fire alarms and sprinkler systems. It was found that sprinkler systems, across all levels of fire severity, were mostly only partially operational. Fire alarm systems were successfully activated in low and medium-severity fire incidents but there is room for improvement in higher-severity incidents.

The Office of the Fire Marshal (OFM) plays a crucial role in overseeing fire safety regulations in Ontario, ensuring fire protection, prevention, and public safety across the province. It is responsible for setting safety standards and improving public safety in all municipalities. Under the leadership of the OFM, municipal fire departments, such as Toronto Fire Services (TFS), allocate resources and develop plans to protect our communities [@ontario_fire_marshal]. Therefore, by analyzing data within their standards, identifying factors that contribute to high-risk fire incidents, and evaluating current operational mechanisms, TFS can gain valuable insights to develop strategic protocols that prioritize certain incidents. These findings will help both the public and fire departments enhance response to incidents, allocate resources more effectively, and reduce the occurrence of controllable incidents.

The remainder of this paper is structured as follows. Data [@sec-data] describes the data used in the analysis, including the sources and key variables along with their visualizations. Model [@sec-model] outlines the models applied to estimate fire severity and identify key factors. Result [@sec-results] presents the analysis results with supporting graphs and tables. Discussion [@sec-discussion] provides interpretations of the results in the context of current fire safety practices and suggests future improvements. Finally, Appendix [@sec-appendixa] includes details about variable selection criterion and data cleaning, with the visualization of all variables in the analysis dataset; Appendix [@sec-appendixb] is a supplementary datasheet that explains the building of the analysis dataset; Appendix [@sec-appendixc] outlines my idealized methodology for conducting this study, given a budget of $100K.

In this paper, analyzing with R (@citeR), I used several packages throughout the analysis.  The `tidyverse`[@wickham2023tidyverse], `dplyr`[@wickham2023dplyr], and `tidyr`[@wickham2023tidyr] were used for data manipulation. `ggplot2`[@wickham2016ggplot2] facilitated data visualization, while `lubridate`[@grolemund2011lubridate] was used for date-time handling. `knitr`[@xie2023knitr] helped integrate code into reports. For modeling, I used `rstanarm`[@gelman2020rstanarm] and `modelsummary`[@fox2021modelsummary] for summaries, `caret`[@kuhn2023caret] for model training, and `xgboost`[@chen2016xgboost] for boosting. Additional tools like `rpart`[@therneau2015rpart], `fitdistrplus`[@delignette-muller2023fitdistrplus], and `pROC`[@robin2023pROC] supported partitioning, distribution fitting, and model evaluation.

# Data {#sec-data}

## Overview

I use the statistical programming language R [@citeR]to investigate data about fire incidents, downloaded from the Open Data Toronto Portal [@OpenDataToronto], named [Fire Incidents](https://open.toronto.ca/dataset/fire-incidents/) [@TFSfireedata]. The dataset was last updated on December 31, 2023. It includes fire incidents following the Ontario Fire Marshal's (OFM) definition, with descriptions according to OFM standards. The dataset also provides detailed information on Toronto Fire Services (TFS) responses to each fire incident. The data is mostly complete, though some incidents are still under investigation. Feature includes specification on the origin of fires, impacts on civilians and buildings, as well as geographical data (see @tbl-preview-data). Additionally, it contains details on TFS response times and actions taken. Overall, the dataset is comprehensive, informative, and provides valuable insights for my later study.

```{r}
#| label: tbl-preview-data
#| tbl-cap: Preview of the Fire Incident data (https://open.toronto.ca/dataset/fire-incidents/) before cleaning, displaying the first four incidents with columns decided first in interested for study.
#| echo: false
#| warning: false
...
The dataset is clearly structured, with the first column as ‘\_id,’ which is a unique identifier for each record in the database. There are 43 columns in total in the original set, containing information ranging from incident type to infrastructure's presence. These columns can be divided into four categories based on my understanding:

1.  **Incident Type and Cause**: This category provides information directly related to each fire incident, including descriptive details about the incident, its nature, and origin. It covers aspects such as where the fire started, the materials involved, and potential causes. These variables are crucial for predicting fire behavior, and include columns like 'area of origin,' 'level of origin,' and 'smoke spread.'
...
variable_table <- data.frame(
  Variable = c(
    ...
    "Ignition_Source", 
    "Number_of_Responding_Appliances", 
  ...
  Description = c(
    "A unique identifier for each record, essential for distinguishing between incidents and for tracking and referencing.",
    "Represents the fire’s origin location, useful for analyzing geographical patterns and fire causes.",
    "Indicates whether the building was occupied, under construction, or vacant, providing insights into fire severity and response needs.",
    "Classifies the economic impact of the fire on businesses, helping to understand disruptions and guide recovery efforts.",
    "The count of civilian casualties, reflecting the human impact and assessing fire safety and evacuation effectiveness.",
    "The monetary loss due to fire damage, key for assessing the economic burden and informing prevention efforts.",
    "The number of secondary fires caused by the primary incident, useful for evaluating the fire's spread and containment success.",
    "The final classification of the fire (e.g., accidental, arson), critical for identifying causes and informing preventive strategies.",
    "Indicates whether the fire alarm system functioned properly, evaluating the system’s effectiveness in alerting and activating suppression.",
    "The source of ignition (e.g., electrical faults, arson), crucial for identifying fire causes and guiding safety measures.",
    "The number of fire trucks and equipment dispatched, reflecting the scale of the incident and resource allocation.",
    "The number of fire personnel involved, providing insight into the severity of the fire and the required response intensity.",
    "The potential cause of the fire, useful for identifying trends and informing fire prevention and safety campaigns.",
    "Indicates if a sprinkler system was present, highlighting its impact on fire severity and potential casualties.",
    "The time when the fire service was notified, key for analyzing response time and minimizing damage.",
    "The time when the first fire service unit arrived, crucial for evaluating response speed and fire control effectiveness.",
    "The number of firefighter casualties, useful for assessing the safety of firefighting operations and improving risk management."
  )
)
...
The raw dataset was stored in its original CSV format to ensure data integrity and reproducibility, and later analysis data after cleaning and processing was saved into a parquet file using the `write_parquet` function in the `arrow` package [@arrow2024]. After loading and cleaning the data, I retained only the relevant columns and renamed them for consistency. I removed rows with 'Undetermined' ignition sources, ensuring that the dataset focused on the relationship between ignition sources and fire severity following the study topic.

I converted time-related variables to the POSIX format. Moreover, I noticed the presence of quite an amount of missing values (NAs) and NULL values. Given that the dataset contains numerous categorical variables, I considered how to handle them differently for these types. I first examined whether any columns contained too little information to provide valuable insights into fire severity or its predictors. After calculating, I decided to remove columns with more than 20% missing data, carefully considering their relevance to the study.

To maintain data quality, rows with more than 30% missing data are removed. With complete data, a new variable named `Response_Time` is created to calculate the time difference (in minutes) between TFS_Alarm_Time and TFS_Arrival_Time (see @fig-response-time-distribution). This new variable will help analyze fire response efficiency and is a key indicator for the study. As shown in the plot, TFS response times center around 5 minutes, with less variation towards longer times.
...
To understand the distribution of various categorical and numerical variables in the fire incident dataset, I analyzed each variable along with its distribution and reviewed its summary statistics. The goal is to gain insights into the key characteristics of the data, which will support the later stages of analysis and modelling. For a comprehensive overview of all chosen variables, including their visualizations and summaries, refer to Appendix @sec-appendixa. Below are a few important variables considered:

1.  **Area of Origin**:\
    This is a categorical variable that indicates the location within a building or structure where a fire started. Initially, this variable had over 100 categories, so the goal was to categorize these locations into broader groups for easier interpretation and analysis. I grouped the areas into categories such as "Residential and Living Spaces," "Cooking and Dining Areas," and "Storage and Utility Areas," among others. These groups were then visualized to better understand the most common fire origins within the dataset (see @fig-area-origin-distribution). Among all categories, the Cooking and Dining area stands out to be the area which most fires originate from. Living space and Transportation areas also have a great amount of incidence occurred.
...
2.  **Civilian Casualties**:\
    This is a numerical variable that provides insight into the severity of the incidents. Basic descriptive statistics were calculated using the `summary()` function. Additionally, a scatter plot was created to explore the relationship between the number of civilian casualties and the incident ID (see @tbl-civilian-casualties-summary). We can see that most of the time, there are no casualties. This data set ranges across over 10 years (see @fig-fire-incidents-by-year in @sec-appendixa), looking at all levels of casualties, and taking its average over 6 years, we can see that this indicates the city has done a good job in protecting civilians from fireplaces.
...
3.  **Final Incident Type**:\
    This is a categorical variable that classifies the type of incident (e.g., fire, explosion). A bar plot was created to visualize the distribution of different incident types.(see @fig-final-incident-type-distribution). It's clear that there is more fire than explosion.
...
4.  **Ignition Source**:\
    This variable provides information about the cause of the fire. Categories of ignition sources, such as "Incandescent Lamp" or "Vehicle - Mechanical," were listed. The distribution of ignition sources was visualized with a bar plot (see @fig-grouped-ignition-source-distribution). We can tell that fire-related and open flame is one of the biggest sources of fire incidence; cooking and heating equipment are second place as sources, which aligns with @fig-area-origin-distribution result, where Ketchen is the place most fire incidence starts off.
...
ggplot(severity_table, aes(x = loss_group, y = casualty_group, fill = count)) +
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 20, hjust = 0.5,size =6)
  ) 
...
A logistic regression model was chosen because the outcome variable, Severity_high, is binary (0/1), and logistic regression is appropriate for binary classification tasks. Especially here, we are working with categorical predictor variables. The logistic regression model used the following predictors. The logistic regression model was initially fitted to the training dataset using the formula: ***Severity_high \~ area_of_origin_grouped + ignition_source_grouped.*** This initial model can be expressed as:
...
The logistic regression model has several limitations. First, it only considers predictors such as area of origin and ignition source, excluding potentially significant factors like weather conditions and building materials, which could affect fire severity. Additionally, the model assumes that the relationships between predictors and severity are linear, which may not fully capture the complexity of fire incidents. Most importantly, the categorization of severity is based on predetermined thresholds, which may not be universally applicable, as different contexts may require different severity definitions. Lastly, the model's predictive accuracy is influenced by the quality of the data.

Notably, there is a slight class imbalance in the dataset, and high-severity incidents are underrepresented. In the training dataset, only 74 out of 10,237 records are classified as high severity. This imbalance can affect the model’s accuracy and its ability to predict high-severity incidents effectively.

## Model Justification

The initial logistic regression model was tested for the significance of the predictors using the p-values from the model summary. Since a high p-value suggests that the variable does not significantly contribute to the model in prediction, it was removed to simplify the model and improve interpretability. This decision also follows the principle outlined in Occam's Razor [@blumer1987occam], which proposes that simplicity is preferred. 
...
The logistic regression model was fitted to predict the probability of high-severity fire incidents based on the categorical variables `Area of Origin`. The final model keeps only significant predictors based on p-values and colinearity diagnostics. Below is a summary @fig-logisticresults-summary of the model's estimates. The coefficients of each predictor. It also included other information about model evaluation. It's noticeable that RMSE is not ideal, but this is already an improvement from the initial model.
...
#| fig-cap: This shows a proportional distribution of fire alarm system operation status across different fire severity levels (Low, Medium, High). This stacked bar plot shows the relative frequencies of various fire alarm system operations within each severity group. Can noctice that fire alarm was not always operated in high severity incidents, but not active in low severity fire.
...
# Variable Selection and Visualization {#sec-appendixa}
...
ggplot(fire_data, aes(x = number_of_responding_apparatus)) +
  ...
  ylab("Frequency") +
  stat_bin(aes(label = ..count..), geom = "text", vjust = -0.5, size = 2)
...
#| tbl-cap: A comprehensive table showing top 6 possible fire casues.
#| fig.width: 3
#| fig.height: 2
...
cause_table_df <- as.data.frame(cause_table) |>
  arrange(desc(Freq))
kable(head(cause_table_df), col.names = c("Possible Cause", "Frequency"))
```
...
ggplot(fire_data, aes(x = year)) +
  ...
  geom_text(stat = "count", aes(label = ..count..), vjust = 0, size = 2) + 
...
ggplot(incident_count_by_year_month_ex, aes(x = month, y = count)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1.5, size = 4), 
    axis.text.y = element_text(angle = 45, hjust = 1, vjust = 1.5, size = 4),
...
# Supplemental Datasheet {#sec-appendixb}

A datasheet for the dataset used in this analysis is included with this paper. Please
refer to this link to the [Datasheet](https://github.com/MaggieZ111119/TFS_Toronto_Fire_Incident/blob/main/paper/datasheet.pdf). 


# Idealized Research Methodology {#sec-appendixc}

## Methodology Overview

This study investigates the severity of fire incidents in Toronto to enhance resource allocation and optimize the effectiveness of the Toronto Fire Services (TFS). The primary focus is on understanding key factors contributing to fire severity, including fire origin areas and ignition sources. Additionally, this study explores the effectiveness of public fire response systems. Important components that need to be considered include fire alarms, sprinkler systems, and smoke detection systems, examining their presence during fire incidents and their effectiveness in reducing fire damage [@thomas2002effectiveness]. The methodology integrates data from fire incident records and surveys, incorporating random forest modelling techniques to identify critical patterns and guide recommendations for TFS operations in the future.

## Target Population

The target population for this study involves fire incidents in the city of Toronto. The key focus is on predicting fire severity based on various factors that influence the outcome of fire incidents and understanding the effectiveness of Toronto's fire infrastructure systems during these events. The population will include all reported fire incidents within the city, classified into three levels of severity: Low, Medium, and High. The population will encompass different building types where fire incidents occur, including but not limited to residential buildings, industrial factories, business establishments, and others.

## Sampling Approach

In the part of predicting fire severity, this study’s findings will be based on observational data from real fire incidents that occurred in Toronto. Therefore, there will not necessarily be sampling as in traditional surveys to collect data. Data will be gathered from official fire department records over a time period of 10 years. This time frame is chosen to ensure both inclusivity and relevance to the current state. The dataset will be divided into strata, subgroups of the population that have similar characteristics [@bailey1991strata], based on different building types, with each stratum containing corresponding information, including fire origins, operational states of fire infrastructure systems, severity-related variables, and actions taken by the department.

A survey will be used to understand the current fire infrastructure system. I will employ the stratified sampling method. Stratified sampling involves dividing the target population into distinct strata based on shared characteristics [@neyman1992two], in this case, it would be building type, as explained in the methodology overview paragraph. From each subgroup, a representative sample will be randomly selected to ensure that all relevant perspectives are included. All citizens involved in fire incidents (for example, people caught in the fire, building managers, and neighbours) will have a chance to fill out a form. The survey will inquire about the presence, timeliness, effectiveness, and ease of use of fire infrastructure and fire-reducing facilities, such as public fire extinguishers and fire water taps.

This survey data, when combined with records maintained by the Toronto Fire Services (TFS), will provide valuable insights into the current state of fire infrastructure. It will help identify gaps and prioritize areas where time and resources should be allocated to enhance the system's effectiveness in the future.

### Fire Incident Records:

-   **Data Source**: This is the primary data source for determining fire severities and making predictions about fire outcomes. The data will be sourced from the Toronto Fire Services (TFS) fire incident reports and the Ontario Fire Marshal’s (OFM) system. These records provide detailed information on each fire incident, such as the origin, ignition source, severity, and actions taken by firefighters, as well as details about the fire infrastructure systems involved.

-   **Variables to Collect**: The classification of fire severity will be based on several factors, including the level of damage, the difficulty of firefighting tasks, injuries, fatalities, and property destruction. To predict fire severity features such as the fire origin area, ignition source, fire response system operation, and the time of year will be investigated. Ideally, information on the presence and functionality of fire alarms, sprinkler systems, smoke detection systems, and other fire infrastructure systems will also be noted.

-   **Privacy Protection**: Privacy of participant information and data is important to all observational data collection [@preibusch2013guide]. To protect the privacy of those involved, the data will be anonymized. Specific addresses will be removed to prevent the identification of individuals or locations. Additionally, rare fire incident data will only be included after obtaining the consent of the individuals involved, ensuring their participation is voluntary.

### Survey on Real Experience with Fire Infrastructure Systems:

Surveys will be distributed using a stratified sampling method to individuals involved in fire incidents, ensuring coverage across all levels of severity and building types. These surveys will gather information on the effectiveness of fire response systems, such as fire alarms, sprinkler systems, and smoke detection systems, in reducing damage and protecting properties. The responses will help guide future improvements in fire infrastructure.

#### Participant Recruitment:

Individuals who were directly involved in or reported the fire incident will be asked to provide their contact information, such as phone numbers, to ensure can reach them for participation. In general, building managers will distribute the survey to all individuals within the building who may have experienced or been impacted by the fire incident. This ensures the study reaches a wide range of individuals with relevant insights.

Since fire incidents involve people’s safety, participants are likely to be motivated to share their experiences and provide valuable feedback on fire operations and infrastructure. As a way to express appreciation, participants who complete the survey will be given the opportunity to enter a raffle for a prize, which will be drawn at the end of each year. This incentive is designed to encourage engagement while ensuring that participation remains voluntary [@freeman2007standards].

All participants will also be directed to a Toronto Fire Prevention website (<https://www.toronto.ca/community-people/public-safety-alerts/fire-prevention-inspection-enforcement/>), where they can access additional information and resources related to fire safety [@toronto_fire_prevention_2024].

#### Survey Design:

The survey is designed to gather both qualitative and quantitative data regarding fire safety infrastructure and its effectiveness. The questions are phrased in a neutral, unbiased manner, and all multiple-choice questions will include a "does not apply" or “other” option to ensure inclusivity and clarity for all respondents. To minimize the risk of trauma triggerings [@alessi2023toward], participants will be informed at the beginning of the survey that they may choose not to answer any questions that make them uncomfortable. A reminder of available support resources will be provided at the end of the survey. Participants will also be reminded of their right to quit the survey at any time without any consequences.

-   **Demographic Information**: This section includes questions about the participant’s location, type of building and their knowledge of fire safety systems, providing context for the responses. Sensitive questions about personal can skipped if the participant feels uncomfortable.

-   **Presence and Accessibility**: This section assesses the presence of essential fire safety features, such as fire alarms, extinguishers, sprinklers, and smoke detectors, and evaluates how easily accessible these resources are during a fire incident. These questions are more focused on infrastructure objectively to avoid causing distress.

-   **Suggestions for Improvement**: An open-ended section where participants can suggest areas for improvement or highlight fire safety issues that need attention. This allows respondents to provide insights into how fire safety infrastructure can be enhanced in the future. Since this section encourages participants to share personal stories and thoughts, it will be optional, and participants can choose whether or not to fill it out.

## Data Validation

**Consistency**: To ensure the consistency of the data, I will compare survey responses with the Toronto Fire Services (TFS) fire records for each incident. For instance, I will compare the reported fire origin, as experienced by the individuals involved, with the fire origin recorded by TFS. If discrepancies are found between the two sources, inconsistent data will be removed.

**Missing Data**: If a record is missing important information, such as the area of the fire, building details, or TFS response personnel, the record will be removed to ensure the quality of the dataset. In cases where operational system information is missing, for example, if the TFS report doesn’t specify details on the operation of fire systems of an incident, I will cross-check with survey responses to find the missing data for that specific incident. If there is inconsistency in the fire system status, such as conflicting responses between the survey and TFS data, or if no survey response is available for a particular incident, the operational data will be classified as "unreported" [@stavseth2019handling].

**Category Validation**: To ensure all recorded information aligns with clearly defined and consistent categories. This includes verifying that the fire incident categories, including severity levels, fire origin, and response actions are accurately defined and organized according to standard protocols. I will review the definitions and ensure that terminology is used consistently across all data sources. Additionally, I will ensure that the severity of the fire is defined in a way that is consistent with fire severity classification standards specific to Toronto. For example, the Office of the Fire Marshal of Ontario has set standards for fire prevention and reporting [@ontario_fire_marshal].

## Modeling - Severity Prediction and Recommendations for Agencies and the Public

### Random Forests

The Random Forests model will be used to identify key predictors of fire severity. These models are an ensemble of multiple decision trees, which together provide predictions by combining the results of various decision paths for critical variables under certain thresholds [@breiman2001random]. The model will identify important factors influencing fire severity, and the predictions will include the weighting of factors based on the frequency and severity of past incidents. For example: even though high-severity fires may be less frequent, understanding the factors leading to these incidents is crucial, so weight adjustments will ensure that less frequent but high-impact factors are appropriately considered in the predictions.

### Model Validation

To ensure the models generalize well and prevent overfitting, cross-validation techniques will be applied, this the total data will be separated into subsets, and the model will be rained on some subsets, and tested on others [@berrar2019cross]. This will increase the reliability of the model. The model performance will be assessed using evaluation metrics such as ROC curves [@hoo2017roc] which is a visual demonstration of the model’s sensitivity an specificity level. Other evaluation criteria include accuracy scores will also be included to ensure that the model’s predictions are reliable and effective.

### Applications and Benefits:

The results from the Random Forest model will inform fire agencies in predicting potential fire incidents. By analyzing the time of year and other features, the model can help agencies prepare for periods when high-severity fires are more likely. Additionally, the model can provide guidance on resource allocation. For example, if in a fire incident call, people indicate that a fire originated in a factory near electrical machinery, and the model predicts a high-risk fire in that area, the fire department can allocate more resources to manage the situation effectively. On the other hand, if a fire occurs in a residential building with the origin in a bathroom, and the model predicts a lower severity, fewer resources can be allocated, leading to more efficient use of time and resources.

Knowledge gained from the model can be used to educate the public about areas with higher risks of severe fires. By targeting fire prevention measures and raising awareness in these high-risk locations, the city’s overall fire safety can be improved, reducing damage and loss of life. For example, if fire origins like HVAC systems or electrical equipment, are discovered to be strongly linked to high-severity fires, the fire agency can prioritize fire prevention plans in these areas.

## Survey Analysis - Effectiveness of Fire Infrastructure and Future Improvements

The survey can gather feedback on the presence, timeliness, effectiveness, and ease of use of various fire infrastructure and fire-reducing facilities, such as fire extinguishers, fire hoses, sprinkler systems, and smoke detectors [@thomas2002effectiveness]. The responses will help identify areas where improvements can be made and assist agencies in understanding the current state of fire safety infrastructure.

A combination of survey responses and records maintained by Toronto Fire Services (TFS) will provide valuable insights into the effectiveness of existing fire systems. By analyzing the data through categorizing responses; identifying geographical patterns; correlating feedback with fire severity; and analyzing open-ended responses, I can identify which systems are working well and where additional resources or improvements may be needed. This could include ensuring buildings are equipped with adequate alarm systems, performing regular maintenance on sprinklers, and enhancing other fire safety features.

### Applications and Benefits:

-   The survey result will guide TFS in directing resources and efforts toward improving fire infrastructure to increase public safety and reduce the impact of fire incidents. For example, if the survey indicates that smoke detectors are frequently reported as ineffective or wrongly capturing situations, TFS may focus on ensuring that all buildings are equipped with properly functioning smoke-detecting systems and conducting regular maintenance checks. Also, if the survey highlights that certain districts have insufficient access to fire extinguishers or water hoses, TFS can prioritize the installation of these essential fire safety resources in those locations to better equip both residents and responders in case of an emergency.

## Link to the Survey:

Here is a link to constructed survey: <https://forms.gle/gQcRL3YTL52S9xgR6/>

## **Budget Allocation Breakdown**

**Data Collection (Fire Incident Records & Surveys) - (30% - \$30,000)**: Obtaining fire incident records, anonymizing data, designing surveys, testing surveys, participant recruitment, and the end-of-year raffle.

-   **Participant End-of-Year Raffle - (5% - \$5,000)**: Funded as part of the recruitment budget to encourage survey participation, with prizes distributed at the end of the year.

**Data Processing and Cleaning (15% - \$15,000)**: Cleaning and validating survey responses, hiring professionals for data review, integrating fire records with survey data, and ensuring privacy protection.

**Model Development and Testing (25% - \$25,000)**: Developing random forest models, training and testing models, and cross-validation to ensure accuracy.

**Analysis and Reporting (20% - \$20,000)**: Analyzing survey data and model outputs, conducting research to generate actionable insights, and generating comprehensive reports.

**Project Management (10% - \$10,000)**: Coordination of the project, hiring personnel, managing stakeholders, and overseeing timelines.
